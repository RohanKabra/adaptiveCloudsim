...
//		}
	...
	public static void createVmsActual(int run, AbstractWorkflowScheduler scheduler) throws InterruptedException {
		// Create VMs
		...
		{
			int stragglerNumber = (int) Math.ceil( actualArrival)<scheduler.getIdleTaskSlots().size()?(scheduler.getIdleTaskSlots().size()-(int) Math.ceil( actualArrival)):0;
			//Log.printLine("abcd\n");
...
//					    //List<VM> result = new ArrayList<Task>();
						...
						Log.printLine(" VM Id : " + vm.getId() + "Host : "+vm.getHost() );
						while( iter.hasNext() )
						{   
