 ...
 */
public class CloudletSchedulerDynamicWorkloadTimeShared extends CloudletSchedulerTimeShared {
	 ...
	 */
	public CloudletSchedulerDynamicWorkloadTimeShared(double mips, int numberOfPes) {
		super();
		setMips(mips);
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			...
			double utlztn = rcl.getCloudlet().getUtilizationOfCpu(currentTime);
			if (utlztn >= 0.7)
	...
	public double getTotalCurrentAvailableMipsForCloudlet(ResCloudlet rcl, List<Double> mipsShare) {
		...
		int pesInUse = 0;
		for (ResCloudlet rcl1 : getCloudletExecList()) {
			pesInUse += rcl1.getNumberOfPes();
		}

		if (pesInUse > currentCPUs) {
			totalCurrentMips /= pesInUse;
		} else {
			totalCurrentMips /= currentCPUs;
		}
